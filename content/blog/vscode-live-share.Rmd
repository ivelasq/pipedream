...
date: "2021-02-03"
categories: ["Collaboration", "R"]
tags: ["devtools", "dplyr", "languageserver", "purrr", "tibble", "usethis", "vctrs", "vscDebugger"]
...
<center>
![](https://ivelasq.rbind.io/img/vscode/mutagen.png)
</center>
...
6. In RStudio, install the required R packages for VS Code, namely [{languageserver}](https://cran.r-project.org/package=languageserver) from CRAN and the latest GitHub release of [{vscDebugger}](https://github.com/ManuelHentschel/vscDebugger):
...
5. To configure your `.Rprofile`, type `code ~/.Rprofile` in your terminal, or if you are already set up with {usethis} in RStudio, run the R command `usethis::edit_r_profile()`. At a minimum, you will want to enable {rstudioapi} [emulation](https://github.com/Ikuyadeu/vscode-R/wiki/RStudio-addin-support#installing-addins) in VS Code:
...
3. In VS Code, open the {mutagen} folder using File > Open… (`Command-O`) and selecting the `mutagen` folder in the Desktop. This will open a new VS Code instance and the title bar will read `Welcome — mutagen`. The R extension icon will be visible on the sidebar since it will detect the `.R` file in the folder.
...
We are huge fans of RStudio and the RStudio IDE. On our wish list for RStudio is that a live collaboration feature be supported in future versions of RStudio Desktop (not only the Pro version) and RStudio Cloud. Having many excellent options is a good thing! In the meantime, it is clear that VS Code is maturing into a capable IDE for R. Live Share is an attractive and free feature that enables real-time collaboration for the masses that we hope others will enjoy as much as we have.
