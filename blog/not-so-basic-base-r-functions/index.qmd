...
There's a lot of [love](https://www.rostrum.blog/posts/2023-10-17-nhs-r-2023/) for base R, and I am excited to pile on. In this blog post, we will explore a few of my favorite not-so-basic base R functions. Click 'Run code' to see them in action, made possible by [webR](https://docs.r-wasm.org/webr/latest/)![^longnote]
...
If you're writing functions, then `invisible()` can be very helpful. In fact, many of your favorite functions use `invisible()`. For example, `readr::write_csv()` [invisibly returns](https://github.com/tidyverse/readr/blob/e4d640ace3bb6ae095a1114158854a38a1da3a30/R/write.R#L214) the saved data frame.

Let's see another example. Run the chunk below to install the purrr and tidytab packages. Tidytab is compiled into a WebAssembly binary on [R-universe](https://gvelasq.r-universe.dev/tidytab#) and `mount = FALSE` is due to a bug in the Firefox WebAssembly interpreter. If you're not using Firefox, then feel free to try without that argument!
