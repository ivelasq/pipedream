...
There's a lot of [love](https://www.rostrum.blog/posts/2023-10-17-nhs-r-2023/) for base R, and I am excited to pile on. In this blog post, we will explore a few of my favorite not-so-basic base R functions. Click 'Run code' to see them in action, made possible by [webR](https://docs.r-wasm.org/webr/latest/) and the [quarto-webr extension](https://quarto-webr.thecoatlessprofessor.com/)![^longnote]
...
Below, we have examples where the functions return their argument `x`, but one does so invisibly.
...
Using purrr and `tidytab::tab2()` together results in two `NULL` list items that we do not need.
...
When writing a function, R can print a lot of stuff implictly. Using `invisible()`, you can return results while controlling what is displayed to a user, avoiding cluttering the console with intermediate results. In fact, many of your favorite functions use `invisible()`, such as `readr::write_csv()`, which [invisibly returns](https://github.com/tidyverse/readr/blob/e4d640ace3bb6ae095a1114158854a38a1da3a30/R/write.R#L214) the saved data frame. 

For example, `invisible()` can help with generating intermediate diagnostics. Per the [Tidyverse design guide](https://design.tidyverse.org/out-invisible.html), "if a function is called primarily for its side-effects, it should invisibly return a useful output". You can use `invisible()` when you want to understand how your function is working by printing results but then hiding the details in the final version of the function.
...
"[Isabella's blog](https://ivelasq.rbind.io)" |> (\(.) {
  text <- sub("\\].*", "", sub(".*\\[", "", .))
  url <- sub("\\).*", "", sub(".*\\(", "", .))
  cat(noquote(paste0('Thanks for checking out <a href="', url, '" target = "_blank">', text, '</a>!')))
})()
```
