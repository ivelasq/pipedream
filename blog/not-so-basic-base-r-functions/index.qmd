...
Let's see another example. First, we'll install an R package, tidytab, which is compiled into a WebAssembly binary on [r-universe](https://gvelasq.r-universe.dev/tidytab#).
...
"[Isabella's blog](https://ivelasq.rbind.io)" |> (\(.) {
  text <- sub("\\].*", "", sub(".*\\[", "", .))
  url <- sub("\\).*", "", sub(".*\\(", "", .))
  HTML(noquote(paste0('Thanks for checking out <a href="', url, '" target = "_blank">', text, '</a>!')))
})()
```
...
[^longnote]: I want to give a big shout out to:
